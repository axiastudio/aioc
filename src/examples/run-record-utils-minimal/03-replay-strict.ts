...
class StrictReplayProvider implements ModelProvider {
  async *stream<TContext = unknown>(
    ...
    yield {
      type: "completed",
      message: "Strict replay completed.",
    };
  }
}

const sourceRunRecord: RunRecord<DemoContext> = {
  runId: "source-run-strict",
  startedAt: "2026-03-11T08:10:00.000Z",
  completedAt: "2026-03-11T08:10:01.000Z",
  status: "completed",
  agentName: "demo-agent",
  providerName: "DemoProvider",
  model: "demo-model",
  question: "Summarize C-42",
  response: "Done",
  ...
  items: [
    ...
    },
    {
      ...
      callId: "source-call-1",
      output: { source: "recorded", tier: "enterprise" },
    },
  ],
  promptSnapshots: [],
  requestFingerprints: [],
...
async function main(): Promise<void> {
  setDefaultProvider(new StrictReplayProvider());

  let liveExecutions = 0;
  const lookupCustomer = tool<DemoContext>({
    name: "lookup_customer",
    description:
      "Live lookup (should not run in strict when output is recorded)",
    parameters: z.object({ customerId: z.string() }),
    execute: async () => {
      liveExecutions += 1;
      throw new Error(
        "Live tool execution should not be called in strict replay.",
      );
    },
  });
